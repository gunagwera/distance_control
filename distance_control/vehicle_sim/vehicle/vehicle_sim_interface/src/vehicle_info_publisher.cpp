...
// added vehicle1 & 2
// func:
// handle controller configs and info update/retr/vehi  
class VehicleInfoPublisher
{
  private:
    ...
    ros::Publisher vehicle_pose_pub_1;
...
VehicleInfoPublisher::VehicleInfoPublisher() : nh_(""), pnh_("~")
{
    ...
    vehicle_pose_pub_1 = nh_.advertise<geometry_msgs::PoseStamped>("/vehicle1/pose", 1, true);
    ...
    pnh_.param("ns", ns_, std::string("vehicle1"));
    pnh_.param("ns", ns_, std::string("vehicle2"));
    pnh_.param("ns", ns_, std::string("vehicle3")); 
    pnh_.param("ns", ns_, std::string("vehicle4"));
...
void VehicleInfoPublisher::publishTimerCallback(const ros::TimerEvent &e)
{
    ...
    output_pose.header.frame_id = ns_ + "base_link";
    ...
    vehicle_pose_pub_1.publish(output_pose);
